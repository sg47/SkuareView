//****************************************************************************/
//
//  @file: fits_to_j2k.cpp
//  Project: fits_to_j2k
//
//  @author Slava Kitaeff
//  @date 29/07/12.
//  @brief The utility to convert astyronomy FITS spectral-image data-cubes 
//  @brief to JPEG2000
//  Copyright (c) 2012 University of Western Australia. All rights reserved.
//
// This code is based on original 
// kdu_compress.cpp [scope = APPS/COMPRESSOR]
// Version: Kakadu, V7.0
  ...
  print_usage(char *prog, bool comprehensive=false)
{
  ...
  if (comprehensive)
    out << "\tOne or more input files. If multiple files are provided, "
           ...
           "PPM, and FITS, as determined by the file suffix.  For raw files, "
  ...
  out << "-u -- print a brief usage statement.\"\n";
    ...
    //This section if specificaly for FITS input files and benchmarking
    
  out <<  "-iplane {<start>,<end>} -- argument requires a comma-"
          "separated first and last frequency plane parameters to read.";
  out <<  "-istok {<start>,<end>} -- argument requires a comma-"
          "separated first and last Stok parameters to read.";
  out << "-scale -- scaling to perform on raw FITS data (such as LOG, "
    "NEGATIVE_LOG, RAW, NEGATIVE_RAW, LINEAR, NEGATIVE_LINEAR, SQRT, "
    "NEGATIVE_SQRT, POWER, NEGATIVE_POWER, SQUARED, NEGATIVE_SQUARED).  "
    "Not all transforms are supported for all FITS file types.\n";
  out << "-LL -- write losslessly compressed JPEG 2000 image(s) in addition "
    "to the (possibly) lossy output\n";
  out << "-CB -- perform compression benchmarking.  Only produces accurate "
    "results if all planes and stokes of a data cube are converted.\n";
  out << "-QB -- perform and display all quality benchmarks.  Benchmarks are "
    "calculated for each plane.  Takes precedence over -QB_* options specifying"
    "individual tests.\n";
  out << "-QB_FID -- perform and display fideliy quality benchmark\n";
  out << "-QB_PSNR -- perform and display peak signal to noise ratio quality "
    "benchmark\n";
  out << "-QB_MAD -- perform and display maximum absolute distortion quality "
    "benchmark\n";
  out << "-QB_MSE -- perform and display mean squared error quality benchmark\n";
  out << "-QB_RMSE -- perform and display root mean squared error quality "
    "benchmark\n";
  out << "-QB_MAE -- perform and display mean absolute error quality benchmark\n";
  out << "-QB_SE -- perform and display squared error sum quality benchmark\n";
  out << "-QB_AE -- perform and display absolute error sum quality benchmark\n";
  out << "-QB_SI -- perform and display uncompressed squared intensity sum "
    "quality benchmark. Note: if -noise is present, quality benchmarks are "
    "performed relative to the image WITH noise added.\n";
  out << "-QB_RES -- write residual image\n";
  out << "-noise -- add Gaussian noise to image pixel intensities to give "
    "a specified PSNR\n";
  out << "-noise_field -- write the noise field added as a result of the -noise "
    "parameter to a file\n";
  out << "-noise_pct -- add Gaussian noise to raw FITS values with a standard "
    "deviation specified as a percentage of the range of FITS values\n\n";
  ...
  try { 
    if ((flush_period > 0) && (flush_period < INT_MAX))
      { // Set up `auto_flush'.  We have to go to a bit of effort here to
        ...
        kdu_long ll_trigger_interval = 0;
        if ((flush_period+(flush_period>>1)) < max_tile_lines)
          { // Otherwise, don't bother with incremental flush within tile
            ll_trigger_interval = flush_period * min_sub_y;
            ll_trigger_interval *= num_comps;
            ll_trigger_interval *= tile_indices.size.x;
            if (ll_trigger_interval > (1<<30))
              ll_trigger_interval = 1<<30; // Just in case
          }
        
        codestream.auto_flush((int)tc_trigger_interval,
                              (int)tc_trigger_interval,
                              (int)ll_trigger_interval,
                              (int)ll_trigger_interval,
              ...
              if (comp->ratio_counter < 0)
                {
                  ...
                  if (comp->reader.exists())
                    comp->reader.get(n,*(comp->line),x_tnum);                   //<--- actual input image line reading
                }
            }
        }
      if (all_done)
...
int main(int argc, char *argv[])
{
  ...
  kdc_file_binding *inputs =
    parse_simple_args(args,ofname,record_stream,transpose,vflip,hflip,
                      ...
                      num_jpx_layers,num_threads,num_xform_threads,
  ...
  else
    {
      ...
      sample_processing_bytes =
        compress_multi_threaded(codestream,tile_indices,inputs,roi_source,
                                layer_bytes,num_layer_specs,layer_thresholds,
                                !no_info,rate_tolerance,allow_shorts,
                                flush_period,num_threads,num_xform_threads,
