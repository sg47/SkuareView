     ...
     `buf_dims'. */
{
  ...
  if (!simd_erase_region(cbuf,erase_dims.size.y,erase_dims.size.x,
                         row_gap,bgnd))
    ...
    { 
      ...
      for (m=erase_dims.size.y; m > 0; m--, cbuf+=row_gap)
        for (dp=cbuf, n=erase_dims.size.x; n > 0; n--)
          *(dp++) = bgnd;
    }
}
      
/******************************************************************************/
/* STATIC                erase_float_buffer_region                            */
/******************************************************************************/
...
#   endif // KDU_SIMD_OPTIMIZATIONS
    { 
      int m, n;
      float *dp, b0=fbgnd[0], b1=fbgnd[1], b2=fbgnd[2], b3=fbgnd[3];
      for (m=erase_dims.size.y; m > 0; m--, cbuf+=row_gap)
        for (dp=cbuf, n=erase_dims.size.x; n > 0; n--, dp+=4)
...
#ifdef KDU_SIMD_OPTIMIZATIONS
  if (!simd_copy_region(dst,src,copy_dims.size.y,copy_dims.size.x,
                        dst_row_gap,src_row_gap))
#endif // KDU_SIMD_OPTIMIZATIONS
    { 
      int m, n;
      kdu_uint32 *sp, *dp;
      for (m=copy_dims.size.y; m > 0; m--, src+=src_row_gap, dst+=dst_row_gap)
        for (sp=src, dp=dst, n=copy_dims.size.x; n > 0; n--)
          *(dp++) = *(sp++);
    }
}

/******************************************************************************/
/* STATIC                  copy_float_buffer_region                           */
/******************************************************************************/
...
static void
  copy_float_buffer_region(float *dst, int dst_row_gap, kdu_dims &dst_dims,
                           ...
                           kdu_dims &copy_dims)
  /* As above, but for floating-point buffers. */
{
  assert(((copy_dims & src_dims) == copy_dims) &&
         ((copy_dims & dst_dims) == copy_dims));
  ...
  if (!simd_copy_region(dst,src,copy_dims.size.y,copy_dims.size.x,
    ...
    { 
      ...
      float *sp, *dp;
      for (m=copy_dims.size.y; m > 0; m--, src+=src_row_gap, dst+=dst_row_gap)
        for (sp=src, dp=dst, n=copy_dims.size.x; n > 0; n--, sp+=4, dp+=4)
          { dp[0]=sp[0]; dp[1]=sp[1]; dp[2]=sp[2]; dp[3]=sp[3]; }
    }
}

/******************************************************************************/
/* STATIC                    rcopy_buffer_region                              */
/******************************************************************************/
     ...
     values that have not yet been copied. */
{
  ...
  dst += (copy_dims.pos.x + copy_dims.size.x - dst_dims.pos.x)
    + dst_row_gap * (copy_dims.pos.y + copy_dims.size.y - 1 - dst_dims.pos.y);
     ...
     // beyond the first sample to be copied (bottom right sample of the region)
#ifdef KDU_SIMD_OPTIMIZATIONS
  if (!simd_rcopy_region(dst,src,copy_dims.size.y,copy_dims.size.x,
                        dst_row_gap))
#endif // KDU_SIMD_OPTIMIZATIONS
    { 
      int m, n;
...
static void
  rcopy_float_buffer_region(float *dst, int dst_row_gap, kdu_dims &dst_dims,
                            float *src, int src_row_gap, kdu_dims &src_dims,
                            kdu_dims &copy_dims)
  /* Same as `rcopy_buffer_region' but for floating point samples. */
{
  assert((dst == src) && (dst_row_gap == src_row_gap));
  assert(((copy_dims & src_dims) == copy_dims) &&
         ((copy_dims & dst_dims) == copy_dims));
...
static void
  blend_buffer_region(kdu_uint32 *dst, int dst_row_gap, kdu_dims &dst_dims,
                      kdu_uint32 *src, int src_row_gap, kdu_dims &src_dims,
                      kdu_dims &blend_dims)
  /* Does regular alpha-blending.  This function assumes that `erase_dims'
...
#ifdef KDU_SIMD_OPTIMIZATIONS
  if (!simd_blend_region(dst,src,blend_dims.size.y,blend_dims.size.x,
                         dst_row_gap,src_row_gap))
#endif // KDU_SIMD_OPTIMIZATIONS
    { 
      int m, n;
      ...
      for (m=blend_dims.size.y; m > 0; m--, src+=src_row_gap, dst+=dst_row_gap)
        for (sp=src, dp=dst, n=blend_dims.size.x; n > 0; n--)
          { 
...
/******************************************************************************/
/* STATIC                  blend_float_buffer_region                          */
/******************************************************************************/
  ...
  /* As above, but for floating-point buffers. */
{
  ...
  if (!simd_blend_region(dst,src,blend_dims.size.y,blend_dims.size.x,
    ...
    { 
      ...
      for (m=blend_dims.size.y; m > 0; m--, src+=src_row_gap, dst+=dst_row_gap)
        for (sp=src, dp=dst, n=blend_dims.size.x; n > 0; n--, sp+=4, dp+=4)
        { 
          alpha = sp[0];
...
/******************************************************************************/
/* STATIC                  pre_blend_buffer_region                            */
/******************************************************************************/
  ...
  /* Same as `blend_buffer_region', but does pre-multiplied blending. */
{
  ...
  dst += (blend_dims.pos.x - dst_dims.pos.x)
    + dst_row_gap * (blend_dims.pos.y - dst_dims.pos.y);
#ifdef KDU_SIMD_OPTIMIZATIONS
if (!simd_blend_premult_region(dst,src,blend_dims.size.y,blend_dims.size.x,
                               dst_row_gap,src_row_gap))
#endif // KDU_SIMD_OPTIMIZATIONS
  { 
    int m, n;
...
/******************************************************************************/
/* STATIC               pre_blend_float_buffer_region                         */
/******************************************************************************/
...
static void
  ...
  /* Same as `blend_float_buffer_region', but does pre-multiplied blending. */
{
  assert(((blend_dims & src_dims) == blend_dims) &&
         ((blend_dims & dst_dims) == blend_dims));
  src += (blend_dims.pos.x - src_dims.pos.x)*4
     ...
     `factor' / 256, inverting colour channels if `factor' < 0. */
{
  ...
  + dst_row_gap * (blend_dims.pos.y - dst_dims.pos.y);
#ifdef KDU_SIMD_OPTIMIZATIONS
  if (!simd_scaled_blend_region(dst,src,blend_dims.size.y,blend_dims.size.x,
    ...
    { 
      ...
      kdu_int32 aval, red, green, blue, diff, alpha;
      if (factor < 0)
        { xor_mask = 0x00FFFFFF; factor = -factor; }
      for (m=blend_dims.size.y; m > 0; m--, src+=src_row_gap, dst+=dst_row_gap)
        for (sp=src, dp=dst, n=blend_dims.size.x; n > 0; n--)
          { 
            src_val = xor_mask ^ *(sp++); tgt_val = *dp;
            alpha = kdrc_alpha_lut[src_val>>24];
            alpha = (alpha * factor) >> 6;
            if (alpha > (1<<15))
              alpha = 1<<15;
            aval =  (tgt_val>>24) & 0x0FF;
            red   = (tgt_val>>16) & 0x0FF;
...
static void
  mod_blend_float_buffer_region(float *dst, int dst_row_gap, kdu_dims &dst_dims,
                                float *src, int src_row_gap, kdu_dims &src_dims,
     ...
     `factor' / 256, inverting colour channels if `factor' < 0. */
{
  ...
  dst += (blend_dims.pos.x - dst_dims.pos.x)*4
    + dst_row_gap * (blend_dims.pos.y - dst_dims.pos.y);
  if (factor >= 0.0F)
    { // Need to scale overlay alpha channel by `factor'
#ifdef KDU_SIMD_OPTIMIZATIONS
      if (!simd_scaled_blend_region(dst,src,blend_dims.size.y,blend_dims.size.x,
                                    dst_row_gap,src_row_gap,factor))
#     endif // KDU_SIMD_OPTIMIZATIONS
        { 
          ...
          for (m=blend_dims.size.y; m > 0; m--,
               src+=src_row_gap, dst+=dst_row_gap)
            for (sp=src, dp=dst, n=blend_dims.size.x; n > 0; n--, sp+=4, dp+=4)
              { 
                alpha = sp[0] * factor;
    ...
    { // Need to scale overlay alpha channel -`factor' and also invert
      // overlay colour channels.
      factor = - factor;
#ifdef KDU_SIMD_OPTIMIZATIONS
      if (!simd_scaled_inv_blend_region(dst,src,blend_dims.size.y,
                                        blend_dims.size.x,dst_row_gap,
                                        src_row_gap,factor))
#     endif // KDU_SIMD_OPTIMIZATIONS
        { 
          int m, n;
     ...
     skipped in this case. */
{
  ...
  if ((old_buffer == buffer) &&
    ...
    { // Need to copy in bottom-to-top, right-to-left order to avoid overwriting
      ...
      for (edims.pos.y=regmin.y; edims.pos.y < reglim.y;
        ...
        { // Walk through top, centre and bottom stip that might need erasing
          ...
          for (edims.pos.x=regmin.x; edims.pos.x < reglim.x;
            ...
            { // Walk through left, centre, right blocks that might need erasing
              if ((edims.pos.x >= comlim.x) || !mid_strip)
                ...
                { // intersecting block that we have already copied; don't erase
                                   ...
                                   bool mark_all_segs_for_processing)
{
  ...
  for (; idx.y != y_lim; idx.y += y_inc)
    { 
      ...
      kdrc_overlay_segment **old_sref=old_seg_refs+idx.y*old_seg_indices.size.x;
                         ...
                         int &invalid_scale_code, kdu_dims target_frame)
{
  ...
  result.pos.x = (int) (full_target_dims.pos.x * scale_x);
  result.pos.y = (int) (full_target_dims.pos.y * scale_y);
     // The above two lines very deliberately round the scaled result towards 0
     // rather than to the nearest integer or taking the floor or ceiling
     // function.  In most cases, the `full_target_dims.pos' values are
     // non-negative and rounding towards zero has the effect of preventing
     ...
     // composited result is presented in original orientation); in this case,
                                            ...
                                            double max_scale[])
{
  ...
  if (discard_levels > 0)
    { 
      kdu_coords high_subs = active_subsampling[discard_levels-1];
      if (transpose)
        high_subs.transpose();
                                ...
                                kdu_long &max_packet_samples)
{
  ...
  if (single_component < 0)
    codestream->ifc.apply_input_restrictions(mapping.num_channels,
                                             ...
                                             discard_levels,max_region_layers,
                                             &canvas_region,
                                       ...
                                       kdu_long &max_packet_samples)
{
  ...
  if (scan == NULL)
    stream->get_packet_stats(region,max_region_layers,
    ...
    { // Remove the portion of `region' which is covered by `scan'
      ...
      if (!cover_dims.intersects(region))
        scan->get_visible_packet_stats(stream,region,max_region_layers,
                                       precinct_samples,
          ...
          if (compositing_buffer == NULL)
            { // Must be the only layer; we need to create a separate
              ...
              if (buffer_is_composition_surface)
                { // Have to make current `buffer' the new composition buffer so
                  // the application can continue using the buffer recovered
                             ...
                             kdu_uint32 *erase_background)
{
      ...
      for (edims.pos.y=regmin.y; edims.pos.y < reglim.y;
        ...
        { // Walk through top, centre and bottom strip that might need erasing
          ...
          for (edims.pos.x=regmin.x; edims.pos.x < reglim.x;
               edims.pos.x += edims.size.x)
            { // Walk through left, centre, right blocks that might need erasing
              if (have_alpha_channel || (edims.pos.x >= comlim.x) || !mid_strip)
  ...
  float *src_fbuf = (fcbuf==NULL)?NULL:buffer->get_float_buf(src_row_gap,true);
  if (!have_alpha_channel)
                                          ...
                                          bool *vflip_val, bool *hflip_val)
{
  ...
  if ((transpose_val != NULL) || (vflip_val != NULL) || (hflip_val != NULL))
    { 
      jpx_composited_orientation orientation=scan->layer->get_src_orientation();
      ...
      for (lscan=active_layers; lscan != NULL; lscan=lscan->next)
        if ((!lscan->had_codestream_headers()) &&
          ...
          { 
            ...
            kdu_dims old_buf_dims; lscan->get_buffer_region(old_buf_dims);
            kdu_dims old_lyr_dims; lscan->get_layer_region(old_lyr_dims);
            kdu_dims new_lyr_dims =
            lscan->set_scale(transpose,vflip,hflip,scale,
                             invalid_scale_code,fixed_composition_dims);
            if (!composition_invalid)
              { 
                if ((fixed_composition_dims.is_empty() &&
                     (new_lyr_dims != old_lyr_dims)) ||
                    ((composition_buffer == NULL) &&
                     !new_lyr_dims.contains(total_composition_dims)))
                                                ...
                                                bool working_only)
{
  if ((!working_only) && (queue_head != NULL))
    { 
      region = queue_head->buffer_region;
                                                 ...
                                                 int custom_id_val)
{
  ...
  elt->buffer_region = buffer_region;
  if (composition_buffer != NULL)
    { 
    ...
    { 
      ...
      for (n=0; n < nstrms; n++)
        { 
          ...
          kdu_dims cs_region;
          if (region.is_empty())
            cs_region = stream->find_composited_region(true); // Whole region
          else
            cs_region = stream->map_region(region,true);
  ...
  kdu_region_compositor::update_composition()
{
  ...
  kdu_dims layer_dims;
  kdrc_layer *scan;
  total_composition_dims = kdu_dims();
  for (scan=active_layers; scan != NULL; scan=scan->next)
    { 
      ...
      if (!layer_dims)
        return (have_valid_scale=false);
      total_composition_dims.augment(layer_dims);
    }  

  if (total_composition_dims.is_empty())
    need_separate_composition_buffer = true;
  if (!fixed_composition_dims.is_empty())
    { // The composition needs to be performed wtihin a fixed frame
      kdu_coords min = fixed_composition_dims.pos;
      kdu_coords lim = fixed_composition_dims.size-min;
      min.x = (int)(scale * (double) min.x);
      min.y = (int)(scale * (double) min.y);
      lim.x = (int)(scale * (double) lim.x);
      lim.y = (int)(scale * (double) lim.y);
        /* The above quantities are deliberately rounded towards 0 so as to
           minimize the likelihood that the composition surface will appear
           larger than its composited contents when rendered at reduced
           resolution.  If that happens, we may wind up needing to create
           a separate compositing buffer just to deal with extraneous space
           on the edges of the frame created by differing rounding
           conventions. */
      kdu_dims composited_frame_dims;
      composited_frame_dims.pos = min;
      composited_frame_dims.size = lim-min;
      composited_frame_dims.to_apparent(transpose,vflip,hflip);
      if (!total_composition_dims.contains(composited_frame_dims))
        need_separate_composition_buffer = true;
      total_composition_dims = composited_frame_dims;
    }
