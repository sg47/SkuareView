    ...
    { 
      ...
      if (ack_time > 0)
        { // Implement channel rate/delay estimation algorithm
          ...
          if ((qp->element_in_group == 0) &&
            ...
            { // Update delay estimate based upon `rtt_usecs'
              kdu_long delay_usecs = rtt_usecs -
                estimated_network_usecs_per_byte*qp->started_bytes;
              if (delay_usecs < 10)
                delay_usecs = 10;
          ...
          kdu_long typ_rtt = estimated_network_delay_usecs +
            (qp->window_bytes * estimated_network_usecs_per_byte);
          kdu_long max_delay = max_ack_offset+KD_UDP_RETRANSMIT_FACTOR*typ_rtt;
...
{ 
  estimated_network_usecs_per_byte =
    1 + (kdu_long)(1000000.0 / estimated_network_rate);
  if (rate_unreliable)
    { 
