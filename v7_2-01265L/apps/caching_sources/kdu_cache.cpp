                            ...
                            bool mark_if_augmented)
{
  ...
  if (codestream_id > state->max_codestream_id)
    state->max_codestream_id = codestream_id;
  ...
  for (seg=cache->root; seg->shift > 0; )
    {
      ...
      if (seg->segs[idx] == NULL)
        { 
          kd_var_cache_seg *new_seg = new (std::nothrow) kd_var_cache_seg;
          if (new_seg == NULL)
            { 
              release_lock();
              throw std::bad_alloc();
            }
          new_seg->base_id = base_id;
          new_seg->shift = seg->shift - 7;
          state->total_cache_segs++;
          seg->segs[idx] = new_seg;
        }
      seg = seg->segs[idx];
...
{
  if (attached != NULL)
    return attached->get_max_codestream_id();
  else
    return state->max_codestream_id;
}
                                ...
                                bool *is_complete)
{
  ...
  if (stream != NULL)
    { 
      ...
      kd_cache_buf *buf = NULL;
      if ((seg != NULL) && (seg->shift == 0) && (id < 128) && (id >= 0) &&
          ((buf = seg->databins[id]) != NULL))
        { 
          kd_cache_hd *head = (kd_cache_hd *)(buf->bytes);
          length = head->initial_bytes;
          if (is_complete != NULL)
  ...
  if (stream != NULL)
    { 
      ...
      if ((seg != NULL) && (seg->shift == 0) && (id < 128) && (id >= 0) &&
          ((buf = seg->databins[id]) != NULL))
        { 
          was_marked = ((kd_cache_hd *)(buf->bytes))->marked;
  ...
  if (stream != NULL)
    { 
      ...
      if ((seg != NULL) && (seg->shift == 0) && (id < 128) && (id >= 0) &&
        ...
        { 
          kd_cache_hd *head = (kd_cache_hd *)(buf->bytes);
          buf_pos = (int) sizeof(kd_cache_hd);
          bytes_left = head->initial_bytes;
        }
    }
    ...
    { 
      ...
      if ((seg != NULL) && (seg->shift == 0) && (id < 128) && (id >= 0) &&
        ...
        { 
          ...
          length = state->read_head.initial_bytes;
          if (is_complete != NULL)
            *is_complete = (state->read_head.have_final &&
                            (state->read_head.hole_list==NULL));
        }
    }
