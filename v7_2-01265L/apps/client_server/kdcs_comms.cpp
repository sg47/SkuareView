  ...
  if (scan != NULL)
    {
      ...
      if (servicer != scan->servicer)
        { 
          ...
          else if (scan->on_active_list || scan->on_wakeup_list)
                                    ...
                                    int new_condition_wait_microseconds)
{
  ...
  if (wakeup_refs != NULL)
    { // Perform immediate wakeups only
  ...
  else if ((read_set != NULL) || (write_set != NULL) || (error_set != NULL))
    ...
    { 
      ...
      if (socket_connected)
        { // No need to explicitly recover the sender's address
          msg_len = ::recvfrom(socket->sock,(char *) buf,(size_t) msg_max,
                               msg_flags,NULL,NULL);
#ifdef KDCS_WIN_SOCKETS
...
#endif // !KDCS_BSD_SOCKETS
          msg_len = ::recvfrom(socket->sock,(char *) buf,(size_t) msg_max,
                               msg_flags,(sockaddr *) &addr_buf,&addr_len);
          if ((msg_len > 0) && (peer_address != NULL))
    ...
    { 
      ...
      if (socket_connected)
        { // We will ignore `peer_address'
          result = ::send(socket->sock,(char *) msg,msg_len,0);
        }
      else
        { // Need `peer_address'
          ...
          result = ::sendto(socket->sock,(char *) msg,msg_len,0,
