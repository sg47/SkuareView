...
class kdu_sample_allocator {
  /* [BIND: reference]
     ...
     be over-read by at least 128 bytes.  That is, applications can
     legally read up to 128 bytes beyond the storage that has nominally
     been allocated and also up to 128 bytes before the storage that has
     nominally been allocated.  However, they may not generally write beyond
     the allocated storage without interfering with other buffers allocated
     by the same sample allocator.
     [//]
  ...
  public: // Member functions
    ...
    void restart()
      { 
        ...
        if (alignment_reserved < 128) alignment_reserved = 128;
    ...
    void pre_align(size_t alignment_multiple)
      ...
      /* [SYNOPSIS]
           ...
           might be of interest are 32 (for AVX operands), 64 (common
           L1 cache line boundary) and 128 (common L2 cache line boundary).
      */
    size_t pre_alloc(bool use_shorts, int before, int after,
                     int num_requests=1)
      { 
        pre_align(64); // We may already have stricter alignment
        size_t offset = bytes_reserved;
        int num_samples = ((7+before) & ~7) + ((7+after) & ~7);
        int num_bytes = (use_shorts)?(num_samples<<1):(num_samples<<2);
      ...
      /* [SYNOPSIS]
           ...
           [>>] Entry 0 must be aligned on a multiple of 8 samples (16 bytes
                for 16-bit samples and 32 bytes for 32-bit samples).
           [>>] The space reserved `before' and `after' entry 0 is rounded up
                to an integer multiple of 8 samples.
           [>>] The entire allocated block of memory also satisfies any
                alignment constraints associated with an immediately
                preceding call to `pre_align'.
           [//]
           Remember that you can legally read from (but not write to)
           locations that lie up to 128 bytes before and 128 bytes after
           the nominal allocated memory block.
         [RETURNS]
    ...
    size_t pre_alloc_block(size_t num_bytes)
      ...
      /* [SYNOPSIS]
           ...
           memory is aligned at least on an 8-byte boundary, although the
           default alignment might be stricter as a matter of system-wide
           optimization.  In any case, you can enforce stricter alignment
           by calling `pre_align' prior to this function.
           [//]
           Remember that you can legally read from (but not write to)
           locations that lie up to 128 bytes before and 128 bytes after
           the nominal allocated memory block.
    ...
    kdu_sample16 *alloc16(int before, int after, size_t alloc_off,
      ...
      { 
        assert(!pre_creation_phase);
        assert(!(alloc_off & 63));
        before = (before+7)&~7; // Round up to nearest multiple of 8 samples
        size_t num_samples = before + ((after+7)&~7); // Also rounded up
  ...
  public: // Life cycle functions
    ...
    void pre_create(kdu_sample_allocator *allocator,
      ...
      /* [SYNOPSIS]
           ...
           where L is 16 for 16-bit samples (`use_shorts'=true) and 32 for
           32-bit samples (`use_shorts'=false).  Finally, it is possible to
           read at least 128 bytes beyond the end or before the start of
      ...
      */
        ...
        assert((extend_left <= 248) && (extend_right <= 248));
        this->width=width;
        ...
        buf_after = (kdu_byte)((extend_right+7) & ~7);
    ...
    bool raw_exchange(kdu_sample16 *&raw_buf, int raw_width)
      ...
      /* [SYNOPSIS]
           ...
           octet aligned (16-byte aligned) and span `raw_width' samples,
           rounded up to a whole number of octets.  The exchange succeeds if
    ...
    bool raw_exchange(kdu_sample32 *&raw_buf, int raw_width)
      ...
      /* [SYNOPSIS]
           ...
           array is expected to be aligned on a 32-byte boundary and have
           storage that spans `raw_width' samples, rounded up to a whole
           number of octets.  That buffer should have been allocated using
           the same `kdu_sample_allocator' as the current object.
      */
  // --------------------------------------------------------------------------
    ...
    KDU_EXPORT kdu_long
      ...
      /* [SYNOPSIS]
         ...
         [ARG: buffer_rows]
           ...
           passed to the DWT analysis and block coding machinery.  There are
           two important cases to understand:
    ...
    KDU_EXPORT kdu_long
      ...
      /* [SYNOPSIS]
         ...
         [ARG: buffer_rows]
           ...
           machinery.  There are two important cases to understand:
           [>>] If the DWT processing is performed in-line, in the same thread
