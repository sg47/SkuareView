...
class kd_synthesis : public kdu_pull_ifc_base {
  ...
  /* Notes:
     ...
     aligned on a 16-byte boundary (32-byte boundary for 32-bit/sample
     representations).  This alignment is beyond our control,
     ...
     `x_min_pull' - `x_min_buf' is a multiple of 16 bytes (32 bytes for 32-bit
     samples), so that data transfers from internal to external line buffers
     can be fully aligned on 16 (resp. 32) byte boundaries.
