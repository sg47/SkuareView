  ...
  kd_tile::initialize()
{
  ...
  if (codestream->out != NULL)
    { 
      int tpart_flags, plt_part_flags;
  ...
  kd_tile::generate_tile_part(int max_layers, kdu_uint16 slope_thresholds[])
{
  ...
  while ((p_ref = sequencer->next_in_sequence(p_res,p_idx)) != NULL)
    { 
      ...
      if (!(precinct->flags & KD_PFLAG_READY))
        break; // This precinct has not yet been marked as "ready" for writing
    ...
    { 
      ...
      if (layer_idx < max_layers)
        ...
        { // Write an empty packet
          pbytes = precinct->write_packet(0,true);
        }
  ...
  kdu_tile::close(kdu_thread_env *env)
{
  ...
  if (state->is_open)
  ...
  else
    {
      ...
      if (env != NULL)
        state->codestream->release_lock(KD_THREADLOCK_GENERAL,env);
  ...
  for (b=0; b < num_subbands; b++)
    { 
      ...
      do { // Enter compare-and-set loop
        ...
        if (old_S == 1)
          ...
          new_state = new_R = 0; // Respond to detachment request
               ...
               !band->bkgnd_state.compare_and_set(old_state,new_state));
      ...
      if (new_state == 0)
        ...
        { 
          assert(p_delta > 0);
          queue->update_dependencies(p_delta,0,env); // Closure call
        }
    }
  ...
  kd_resolution::advance_precinct_rows_available(kdu_thread_env *env)
{
  ...
  for (; p_across > 0; p_across--, rel_p_idx+=p_inc)
    { 
      ...
      precinct = precinct_refs[p_num].open(this,rel_p_idx,true);
      if ((codestream->in == NULL) || (precinct == NULL) ||
          (precinct->num_outstanding_blocks.get() == 0) ||
          (precinct->flags & KD_PFLAG_LOADED_LOCKED))
        continue; // Note: `precinct' can be NULL if `kd_precinct::initialize'
...
int
  kdu_subband::get_block_geometry(bool &transpose, bool &vflip, bool &hflip)
{
  kd_resolution *res = state->resolution;
                                     ...
                                     kdu_thread_env *env)
{
  ...
  } while ((old_val != 0) && !state->bkgnd_state.compare_and_set(old_val,2));
  if (old_val == 0)
                                         ...
                                         kdu_thread_env *env)
{
  ...
  if ((state == NULL) || (state->notify_queue != client_queue))
    ...
    { 
      ...
      do { // Enter compare-and-set loop
        old_state = state->bkgnd_state.get();
        if ((old_state & 3) == 0)
        ...
        new_state = old_state + (kdu_int32)(delta_rows_needed<<2);
                          ...
                          kdu_thread_env *env)
{
  ...
  if ((codestream->in != NULL) && !(precinct->flags & KD_PFLAG_LOADED_LOCKED))
    { 
    ...
    { // Work with resources that belong exclusively to this thread
      if (cs->in != NULL)
        { // Release as many resources as we can here
          ...
          kdu_int32 old_cnt=precinct->num_outstanding_blocks.exchange_add(-1);
          assert(old_cnt >= 1);
          if (old_cnt == 1)
        ...
        { 
          block->store_data(result,cs->get_thread_buf_server(env));
          precinct->num_outstanding_blocks.exchange_add(-1);
        }
    }
  else
    { // Single-threaded operation; work with codestream-wide resources
      if (cs->in != NULL)
        { // Release as many resources as we can here.
          ...
          kdu_int32 old_cnt = precinct->num_outstanding_blocks.get_add(-1);
          assert(old_cnt >= 1);
          if (old_cnt == 1)
        ...
        { 
          block->store_data(result,cs->buf_servers);
          precinct->num_outstanding_blocks.get_add(-1);
        }
    }
}
                                   ...
                                   kdu_uint16 layer_thresholds[])
{
  ...
  assert((unique_id >= 0) && (unique_id < res->precinct_indices.area()));
  for (kd_resolution *rp=res-res->res_level; rp != resolution; rp++)
    unique_id += rp->precinct_indices.area();
    ...
    { 
      ...
      if (!empty_packet)
        { // Start the packet
          ...
          if (tile->use_sop)
            { // Generate an SOP marker.
              pbytes += out->put(KDU_SOP);
              pbytes += out->put((kdu_uint16) 4);
              pbytes += out->put((kdu_uint16)tile->sequenced_relevant_packets);
            }
        }
      kd_header_out head(out);
      if (empty_packet)
  ...
  kd_precinct_server::get(int max_blocks, int num_subbands, int max_layers)
{
  ...
  if (scan == NULL)
    {
      scan=new kd_precinct_size_class(max_blocks,num_subbands,this,
                                      buf_server,max_layers);
