...
// The factors below are uesd in SSSE3 implementations of the 9/7 xform where
...
static kdu_int16 ssse3_w97_rem[4] =
...
#ifndef KDU_NO_SSSE3
  if (kdu_mmx_level >= 4)
    { // 128-bit implementation, using SSSE3 intrinsics
      __m128i vec_lambda = _mm_set1_epi16(ssse3_w97_rem[step_idx]);
  ...
  if (kdu_mmx_level >= 1)
    { // 64-bit implementation, using MMX instructions only.
      ...
      if (step_idx == 0)
        ...
        { // Note: we need to make the upfront initializations below volatile,
          // so that the compiler does not optimize them to occur in the
          // previous program block (where `_mm_empty' never gets called).
          // This is an extremely subtle problem which was observed to occur
          // in GCC 4.2 after an enormous debugging effort.
          volatile __m64 roff = _mm_setzero_si64();
          volatile __m64 tmp = _mm_cmpeq_pi16(roff,roff); // Set to all 1's
  ...
  if (kdu_mmx_level >= 4)
    { // 128-bit implementation, using SSSE3 intrinsics
      ...
      if (step_idx == 0)
        ...
        { // Last two steps use the same code
          for (register int c=0; c < samples; c+=8)
            { 
              __m128i val = *((__m128i *)(src1+c));
              val = _mm_add_epi16(val,*((__m128i *)(src2+c)));
