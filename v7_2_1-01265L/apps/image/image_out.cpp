...
kdu_image_out::kdu_image_out(const char *fname, kdu_args &args, kdu_image_dims &dims,
                             int &next_comp_idx, bool &vflip, bool quiet)
{
  const char *suffix;
  ...
  if ((suffix = strrchr(fname,'.')) != NULL)
    {
      if ((strcmp(suffix+1,"pgm") == 0) || (strcmp(suffix+1,"PGM") == 0))
        ...
        out = new fits_out(fname,args,dims,next_comp_idx,quiet); // still implementing
      else if ((strcmp(suffix+1,"H5") == 0) || (strcmp(suffix+1,"h5") == 0))
        out = new hdf5_out(fname,args,dims,next_comp_idx,quiet);
#endif
    }
  if (out == NULL)
    { kdu_error e; e << "Image file, \"" << fname << ", does not have a "
      "recognized suffix.  Valid suffices are currently: "
      "\"bmp\", \"pgm\", \"ppm\", \"tif\", \"tiff\", \"raw\", \"h5\" "
      "and \"rawl\".  "
      "Upper or lower case may be used, but must be used consistently.";
    }
}
