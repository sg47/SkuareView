                               ...
                               bool synthesize_new_request)
{
  ...
  // f) the number of queues that are neither in timed request mode nor
  //    have any requests to send;
  // g) the total amount of excessive lag that must be redistributed to
  ...
  // h) the queue, if any, that should be the next requester; and
  // i) an appropriate value to use for the target duration of any request
      ...
      if (best_queue->just_started)
        { // `req' must be the queue's startup request
          ...
          if (num_queues_waiting_for_replies >
              num_queues_waiting_for_startup_replies)
            return NULL; // We should neither pre-empt unreplied requests on
