...
void
  jp2_palette::get_lut(int comp_idx, float lut[]) const
{
  assert((state != NULL) &&
         (comp_idx >= 0) && (comp_idx < state->num_components));
...
void
  jp2_palette::get_lut(int comp_idx, kdu_sample16 lut[]) const
{
  assert((state != NULL) &&
         (comp_idx >= 0) && (comp_idx < state->num_components));
  kdu_int32 *src = state->luts[comp_idx];
  ...
  jp2_colour::init(const kdu_byte *profile_buf)
{
  ...
  tmp_profile.init((kdu_byte *) profile_buf); // Type cast is safe here,
                                       // because `donate_buffer' arg is false
