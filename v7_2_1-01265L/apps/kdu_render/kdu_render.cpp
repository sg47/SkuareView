     ...
     multi-threaded processing environment. */
{
  ...
  try { // Just so we can control the order in which the multi-threaded
    ...
    compositor.set_thread_env(NULL,NULL); // Retract the multi-threaded
      // processing environment to avoid problems related to the order of
      // destruction in the event an exception was thrown.
    throw;
  }
     ...
     when used with huge images.  No output file is actually written. */
{
  ...
  catch (...)
    {
      if (env_ref != NULL)
        env.cs_terminate(codestream); // Cleanest way to detach codestream
             ...
             // automatically in `kdu_region_compositor' when the
             // multi-threaded processing environment is retracted -- see
             // demos 5 and 6.
      codestream.destroy();
      if (buffer != NULL)
        delete[] buffer;
  ...
  if (env_ref != NULL)
    {
      env.cs_terminate(codestream); // As in the catch statement above.
      env.destroy(); // Not required; done automatically by `env' destructor
    }
  codestream.destroy();
  delete[] buffer;
}
